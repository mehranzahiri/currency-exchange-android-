...
class ConverterFragment : MvvmFragment<FragmentConverterBinding, RateViewModel>(
    ...
    override fun onEveryInitialization(savedBundle: Bundle?) {
        ...
        data.convertDialog.observe(viewLifecycleOwner) {
            it?.let {
                DialCollection.showConvertDialog(
                    requireContext(),
                    it.getString("from", ""),
                    it.getString("to", ""),
        ...
        binding.pickkerWithdraw.typeface =
            Typeface.createFromAsset(requireActivity().assets, "gilory.ttf")
        binding.pickerDeposit.typeface =
            Typeface.createFromAsset(requireActivity().assets, "gilory.ttf")
